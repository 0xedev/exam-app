...
import { categories, getGreeting, getAllQuestionsFromStorage, Category, Question } from "../../lib/util";
  ...
  const CategoryProgress = () => (
    <View style={tw`mb-4 px-2`}>
      <View style={tw`flex-row justify-between items-center mb-2`}>
        ...
        <Text style={tw`text-orange-500 font-bold`}>{userCategories.length}/{REQUIRED_CATEGORIES}</Text>
      </View>
        ...
        >
          ...
          <Text
            style={[
              ...
              userCategories.includes(item.id) ? tw`text-white` : tw`text-gray-800`,
            ]}
          >
            {item.topic}
          </Text>
        ...
        {showTip && (
          <View style={tw`bg-orange-100 p-4 rounded-xl mb-4`}>
            <Text style={tw`text-orange-800 text-center`}>
              Select 2 categories to begin your test
            </Text>
          </View>
        )}
        ...
        {showConfirm && (
          <View style={tw`bg-orange-100 p-4 rounded-xl mb-4`}>
            <Text style={tw`text-orange-800 text-center mb-4`}>
              You've selected {userCategories.length} categories. Ready to begin?
            </Text>
            <View style={tw`flex-row justify-around`}>
              <Pressable
                ...
                onPress={cancelStart}
              >
                <Text style={tw`text-center font-bold text-gray-800`}>Cancel</Text>
              </Pressable>
              <Pressable
                style={({ pressed }) => [
                  tw`p-3 rounded-xl w-1/3`,
